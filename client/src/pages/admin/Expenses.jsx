...
export default function Expenses() {
  ...
  const columns = [
    ...
    { title: '操作', key: 'action', render: (_, r) => (
      <Space>
        ...
        <Popconfirm title="确定删除该记录？" onConfirm={() => handleDelete(r.id)}><Button size="small" type="link" danger>删除</Button></Popconfirm>
      </Space>
    )},
  ];
  ...
  return (
    <div className="page-container">
      <div className="page-header"><h2 className="page-title">日常开销</h2></div>
      ...
      <Modal title={editId ? '修改开销' : '新增开销'} open={modalOpen} onOk={handleSubmit} onCancel={() => setModalOpen(false)} destroyOnClose width={520}>
        <Form form={form} layout="vertical">
          <Form.Item name="name" label="开销名称" rules={[{ required: true, message: '请输入开销名称' }]}><Input /></Form.Item>
          ...
          <Form.Item name="pay_method" label="支付方式" rules={[{ required: true, message: '请选择支付方式' }]}>
            <Select options={payMethods} placeholder="请选择支付方式" />
          </Form.Item>
          <Form.Item name="amount" label="支付金额" rules={[{ required: true, message: '请输入金额' }]}><InputNumber min={0.01} precision={2} style={{ width: '100%' }} /></Form.Item>
